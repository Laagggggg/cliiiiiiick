...
def compute_adaptive_weights(regime: str, family_sharpes: dict[str, float], min_weight: float = 0.05) -> dict[str, float]:
    active = ["trend", "reversion"]
    sharpes = {f: max(0.0, family_sharpes.get(f, 0.0)) for f in active}

    # Apply regime bias: boost the dominant family for the current regime
    if regime == "TRENDING":
        sharpes["trend"] = sharpes["trend"] * 1.5 + 0.10
    elif regime == "RANGING":
        sharpes["reversion"] = sharpes["reversion"] * 1.5 + 0.10
    # AVOID / NEUTRAL: no bias applied, let raw Sharpe ratios decide
