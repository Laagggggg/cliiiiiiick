...
function setTruth(p){const prov=(p.provenance||{});document.getElementById('mode_truth').textContent=p.mode_truth||'SIMULATED FILLS';document.getElementById('data_grade').textContent=p.data_grade||'UNKNOWN (run doctor)';document.getElementById('transport').textContent=p.transport||'POLLING (run websocket monitor)';document.getElementById('primary').textContent=p.provider_primary||(prov.source_primary||'fallback (run monitor)');document.getElementById('secondary').textContent=p.provider_secondary||(prov.source_secondary||'none');const r=(p.reconciliation||(prov.reconciliation)||{});document.getElementById('recon').textContent=(r.passed===undefined||r.passed===null)?'unknown (run broker daemon)':`${r.passed?'PASS':'FAIL'} max_diff_pct=${r.max_diff_pct??'n/a'}`;document.getElementById('fresh').textContent=(p.freshness_label??p.freshness_seconds??prov.freshness_seconds??'UNKNOWN (run doctor)');document.getElementById('last_bar_ts').textContent=(p.last_bar_ts??prov.end??'UNKNOWN (run monitor)');document.getElementById('next_action').textContent=(p.next_action??'Run API Doctor');document.getElementById('demo_warn').textContent=(p.data_grade==='CSV_SAMPLE'?'NOT LIVE DATA: CSV sample in use. Set Alpaca keys and rerun doctor.':'');}
