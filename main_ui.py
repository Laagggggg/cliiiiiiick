...
<h1>OMEGA-QUANT ULTRA — Proof Terminal</h1><p><b>Tracking SPY (S&P 500 ETF)</b></p>
<div class='banner'>LIVE=<b id='live_verified'>NOT LIVE ❌</b> | MODE_TRUTH=<b id='mode_truth'>-</b> | DATA_GRADE=<b id='data_grade'>-</b> | TRANSPORT=<b id='transport'>-</b> | PRIMARY=<b id='primary'>-</b> | SECONDARY=<b id='secondary'>-</b> | RECON=<b id='recon'>-</b> | FRESHNESS=<b id='fresh'>-</b> (<b id='fresh_basis'>-</b>) | LAST_BAR_TS=<b id='last_bar_ts'>-</b> | FEED=<b id='feed_label'>-</b> | WS_HEALTH=<b id='ws_health'>-</b> | LAST_EVENT=<b id='last_event_kind'>-</b> | STALL_S=<b id='stall_seconds'>-</b> | NEXT_ACTION=<b id='next_action'>-</b></div>
...
function drawSpark(){const c=document.getElementById('spark');const o=document.getElementById('spark_overlay');const ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);if(PRICES.length<2){o.textContent='No live ticks yet — run Live Monitor (Polling/Websocket).';return;}o.textContent='';const min=Math.min(...PRICES),max=Math.max(...PRICES);const span=(max-min)||1;ctx.beginPath();PRICES.forEach((v,i)=>{const x=(i/(PRICES.length-1))*c.width;const y=c.height-((v-min)/span)*c.height; if(i===0){ctx.moveTo(x,y);} else {ctx.lineTo(x,y);}});ctx.strokeStyle='#1a73e8';ctx.lineWidth=2;ctx.stroke();}
...
function setTruth(p){const prov=(p.provenance||{});document.getElementById('mode_truth').textContent=p.mode_truth||'SIMULATED FILLS';document.getElementById('data_grade').textContent=p.data_grade||'CSV_SAMPLE';document.getElementById('transport').textContent=p.transport||'POLLING';document.getElementById('primary').textContent=p.provider_primary||(prov.source_primary||'CSV_SAMPLE');document.getElementById('secondary').textContent=p.provider_secondary||(prov.source_secondary||'none');const r=(p.reconciliation||(prov.reconciliation)||{});document.getElementById('recon').textContent=(p.recon_status||((r.passed===undefined||r.passed===null)?'UNKNOWN':(r.passed?'PASS':'FAIL')));document.getElementById('fresh').textContent=(p.freshness_label??p.freshness_seconds??prov.freshness_seconds??'UNKNOWN (run doctor)');document.getElementById('fresh_basis').textContent=(p.freshness_basis||'unknown');document.getElementById('last_bar_ts').textContent=(p.last_bar_ts??prov.end??'UNKNOWN (run monitor)');document.getElementById('next_action').textContent=(p.next_action??'Run API Doctor');document.getElementById('live_verified').textContent=(p.live_verified?'LIVE VERIFIED ✅':'NOT LIVE ❌');document.getElementById('feed_label').textContent=(p.feed_label||'DEMO');document.getElementById('ws_health').textContent=(p.ws_health||'disconnected');document.getElementById('last_event_kind').textContent=(p.last_event_kind||'none');document.getElementById('stall_seconds').textContent=(p.stall_seconds??0);document.getElementById('demo_warn').textContent=((p.data_grade==='CSV_SAMPLE'||p.data_grade==='CACHED')?'NOT LIVE DATA: demo/cached feed in use. Resolve provider/websocket for live data.':'');}
