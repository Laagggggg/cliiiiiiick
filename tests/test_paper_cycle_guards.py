...
class AltCsvProvider(CsvMarketDataProvider):
    def source_name(self) -> str:  # pragma: no cover - trivial override
        return "csv_alt"
...
def test_reconciliation_halt(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "omega_quant.ops.paper_cycle.get_provider_chain",
        lambda: [CsvMarketDataProvider(), AltCsvProvider()],
    )
